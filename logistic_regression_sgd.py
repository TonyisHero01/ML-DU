...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    train_data, test_data, train_target, test_target = sklearn.model_selection.train_test_split(data_bias, target, test_size=args.test_size, random_state=args.seed)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_permutation in batch_permutations:
            ...
            for index in batch_permutation:
                ...
                sig = sigmoid(x)
                gradient += (sig - train_target[index]) * train_data[index]
        ...
        test_accuracy = sklearn.metrics.accuracy_score(test_target, test_data @ weights >= 0)
